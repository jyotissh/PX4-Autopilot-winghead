...
bool FlightTaskContinuousYaw::activate(const trajectory_setpoint_s &last_setpoint)
{
     ...
     PX4_INFO("ContinuousYaw activate called. ret:%d",ret);

    _position_setpoint(0) = _position(0);
    _position_setpoint(1) = _position(1);

    _yawspeed_setpoint = 45.0f*3.1422f/180.0f;
...
{
    PX4_INFO("ContinuousYaw update, z=%.2f",(double)_position(2));
    float diff_z = _position(2) - _origin_z;

    if(diff_z <= -8.0f){ //NED frame ; climbing up
            _velocity_setpoint(2) = 1.0f;
            _yawspeed_setpoint = 45.0f *3.142f/ 180.f *  -1.0f;

    } else if(diff_z >= 0.0f){ // climbing down
        _velocity_setpoint(2) = -1.0f;
